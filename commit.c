...
int commit_create(const char *message, ObjectID *commit_id_out) {
    // Step 1: Build tree from the current index (staged files)
    Commit commit;
    ...
    // Step 4: Serialize commit and write to object store
    void *data;
    size_t len;
    ...
    // Step 5: Update HEAD/branch ref to point to the new commit
